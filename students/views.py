...
def view_logs(request):
    logs = cache.get('user_activity_log', [])
...
class StudentsViewSet(viewsets.ModelViewSet):
    ...
    def second_course_of_studying(self, request):
        students = Students.objects.all().filter(
            Q(birth_date__startswith="2004") | Q(birth_date__startswith="2003")
        )
    ...
    def not_ru_phone_but_gmail_or_mail(self, request):
        students = Students.objects.all().filter(
            ~Q(phone__startswith="7")
            & (Q(email__contains="gmail.com") | Q(email__contains="mail.ru"))
        )
    ...
    def programmers_or_engineers_who_are_in_first_10(self, request):
        students = Students.objects.all().filter(
            (
                    Q(skill__skill_name="Программирование")
                    | Q(skill__skill_name="Инженирование")
            )
            & ~(Q(id__gt=10))
        )
